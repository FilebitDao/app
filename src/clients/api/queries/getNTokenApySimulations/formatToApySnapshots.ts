...
import { NTokenApySnapshot } from './types'

const formatToApySnapshots = ({
  nTokenBalanceCallResults
}: {
  nTokenBalanceCallResults: ContractCallResults
}): NTokenApySnapshot[] => {
  const apySimulations: NTokenApySnapshot[] = []
  const results = Object.values(nTokenBalanceCallResults.results)[0].callsReturnContext
